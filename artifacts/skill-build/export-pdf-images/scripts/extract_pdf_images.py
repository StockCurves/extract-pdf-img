...
Usage:
    python extract_pdf_images.py <pdf_path> [options]
...
Options:
    ...
    --output-stem TEXT           Base prefix for default filenames
    --filename-template TEXT     Filename template with {stem} {kind} {index} {page} {label}
"""
...
def _build_output_name(base_stem: str,
                       kind: str,
                       index: int,
                       page_no: int,
                       label: str,
                       filename_template: str | None) -> str:
    index_text = f"{index:03d}"
    label_text = _safe_filename_part(label)
    ...
    return f"{base_stem}-{kind}{index_text}.png"
...
def extract(pdf_path: Path,
    ...
    """
    stem    = pdf_path.stem
    base_stem = _safe_filename_part((output_stem or stem)[:80])
...
def main():
    ...
    ap.add_argument("--output-stem", default=None,
                    help="Base prefix for default filenames")
    ap.add_argument("--filename-template", default=None,
                    help="Template using {stem} {kind} {index} {page} {label}")
    args = ap.parse_args()
